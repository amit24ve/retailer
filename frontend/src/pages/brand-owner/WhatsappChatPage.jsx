...
function ChannelSettingsPanel({ onClose }) {
  ...
  useEffect(() => {
    api.get('/sms/settings')
      .then((r) => {
        const data = r.data || {};
        setSmsHeader(data.sms_header_mode === 'custom' ? 'custom' : 'mcuben');
        setCustomSenderId(data.custom_sender_id || '');
        setOtpChannel(data.otp_channel || 'sms');
      })
      .catch(() => {});
  }, []);
  ...
  const save = async () => {
    ...
    try {
      await api.post('/sms/settings', {
        sms_header_mode: smsHeader === 'custom' ? 'custom' : 'default',
        custom_sender_id: customSenderId,
              ...
              {smsHeader === 'custom' && (
                <div className="mt-4">
                  ...
                  <input className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-300"
                    placeholder="e.g. MYBRAND" maxLength={11} value={customSenderId} onChange={(e) => setCustomSenderId(e.target.value.toUpperCase())} />
              ...
              <div className="space-y-2.5">
                {[
                  ...
                  <label key={opt.v}
                    ...
                    <input type="radio" name="otp" checked={otpChannel === opt.v} onChange={() => setOtpChannel(opt.v)} className="w-4 h-4" style={{ accentColor: '#a89442' }} />
                    <div>
                      <p className="text-sm font-bold text-slate-900">{opt.label}</p>
                      <p className="text-xs text-slate-400">{opt.desc}</p>
                    </div>
...
export default function WhatsappChatPage() {
  ...
  const sendMessage = () => {
    ...
    const msg = { id: Date.now(), from: 'agent', text: input, time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) };
    setConversations(prev => prev.map(c =>
      c.id === selected.id ? { ...c, messages: [...c.messages, msg], lastMsg: input, time: 'Just now' } : c
    ));
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
            ...
            {selected.messages.map(msg => (
              <div key={msg.id} className={`flex ${msg.from === 'agent' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-xs lg:max-w-md px-4 py-2.5 rounded-2xl shadow-sm text-sm leading-relaxed ${
                  ...
                  {msg.text}
                  <p className={`text-[10px] mt-1 text-right ${msg.from === 'agent' ? 'text-cyan-300' : 'text-slate-400'}`}>{msg.time}</p>
                </div>
              </div>
            ))}
          ...
          <div className="px-4 py-3 bg-white border-t border-slate-200 flex items-center gap-2 flex-shrink-0">
            ...
            <button onClick={sendMessage}
              className="w-10 h-10 bg-cyan-500 hover:bg-cyan-600 text-white rounded-2xl flex items-center justify-center flex-shrink-0 transition-colors shadow-md">
