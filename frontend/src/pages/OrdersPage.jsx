...
function POSModal({ onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Simulate success
      onSuccess({ status: 'success', invoice_number: `INV-${Date.now()}`, customer: { name: form.customer_name || 'Customer', new_tier: 'Gold', points_balance: 1816 }, loyalty_valuation: { points_earned: Math.floor(parseFloat(form.gross_amount || 0) / 10), points_redeemed: form.redeem_points_requested, cashback_wallet_earned: parseFloat(form.gross_amount || 0) * 0.04 }, comms_dispatched: { whatsapp_status: 'queued' } });
    } finally {
      setLoading(false);
    }
  };
